...
import defaultStyles from "./../styles/trb-hf-report.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-hf-report.template.jsx)', function() {

        before(function() {
            TrbHfReport = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbHfReport  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-hf-report-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-hf-report-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-hf-report-title').get(0).innerHTML).to.equal("test title");
            });
        });
