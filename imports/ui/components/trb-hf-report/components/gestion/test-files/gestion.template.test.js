...
import defaultStyles from "./../styles/gestion.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (gestion.template.jsx)', function() {
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#gestion-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#gestion-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#gestion-title').get(0).innerHTML).to.equal("test title");
            });
        });
